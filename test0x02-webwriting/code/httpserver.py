...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.field_name in fields:
        ...
        elif 'grades' in fields: 
            course_id, student_id, grades = form_data['course_id'].value, form_data['student_id'].value, form_data['grades'].value
            ...
            conn = sqlite3.connect('edu.db')
            c = conn.cursor()
            sql = "select grades from studentsinfo where course_id=%s and student_id=%s"%(course_id, student_id)
            c.execute(sql)
